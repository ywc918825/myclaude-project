...
- 已知局限：Open Beauty Facts 以欧美品牌数据为主，国产/小众品牌命中率较低；文字搜索不如条码精确（见下方"扫条码"）。
- 沙盒环境说明：本项目的开发沙盒因出网策略屏蔽了对任意外部域名的访问，无法在沙盒里实测这个接口真实连通；已用 Playwright 拦截请求、按接口真实文档格式mock返回值验证了前端的解析/匹配/填表逻辑。真机测试确认 `world.openbeautyfacts.org` 网站本身在境内可以打开，但这不等于浏览器发起的跨域fetch请求一定能成功（可能是CORS限制而非网络不通）——具体是哪种原因还没有最终确认，需要用户在Chrome里对着接口URL直接访问确认。

## 扫条码 + 批量录入模式（stage3）

- **扫条码**：用 `@zxing/browser`（纯客户端条码识别，基于摄像头视频流实时解码，不依赖浏览器原生 `BarcodeDetector` API 以保证 iOS Safari 等兼容性）实现 `frontend/src/pages/cabinet/BarcodeScanner.jsx`，一个全屏摄像头取景层组件。扫到条码后调用 `getProductByBarcode()`（`GET /api/v2/product/{条码}.json`），这是**精确匹配**（不是文字模糊搜索），所以可信度更高，命中后会直接把产品名称、品牌、已知成分标签都带入表单（仍需用户核对），不像文字搜索那样需要用户从候选列表里手动选。
- **批量录入模式**：添加产品页保存成功后不再跳转回柜子页，而是清空表单、停留在当前页，方便连续扫码/拍照录入一整柜子的产品；页面顶部会显示"已连续添加N件"，点"完成"才手动跳转回柜子页查看结果。编辑模式不受影响，保存后依然直接跳转回柜子页。
- 局限：条码通常印在外包装盒上，产品拆封已久、盒子丢了的情况下就扫不到了，这种情况仍然回退到拍照OCR/手动填写。
